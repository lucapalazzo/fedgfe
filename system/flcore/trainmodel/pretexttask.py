...
class PretextTask (nn.Module):
    ...
    def test_metrics(self, x, y = None):
        print ( "Pretext test metrics not implemented" )
        return x
    
    def pool_output_embedding(self, x, pooled_output_dim):
        if pooled_output_dim != self.patch_count:
            x = x.view(-1,self.patch_count_per_row,self.patch_count_per_row, self.output_dim,)
            x_pooled = F.adaptive_avg_pool2d(
                x.permute(0,3,1,2),   # [b,embedding_dim,14,14]
                output_size=(pooled_output_dim,pooled_output_dim)
                # output_size=self.jigsaw_patch_count_per_row
            ).permute(0,2,3,1)  # [b,4,4,embedding_dim]
            x = x_pooled.reshape(x.size(0),pooled_output_dim**2, self.output_dim)
        return x
    ...
    def save_images ( self, images, patches = None, max_saved = 0, output_filename = None, save_interval = 1 ):

        
        if output_filename in self.last_saved_files:
            if self.last_saved_files[output_filename] + save_interval > time.time():
                return
        ...
        for batch_id in range(B):
            if output_filename is None:
                output_filepath = os.path.join(self.image_output_directory, f"reconstructed_{batch_id}.png")
            else:
                output_filepath = os.path.join(self.image_output_directory, output_filename)
            ...
            save_grid_images(save_images, nrow=len(images), output_path=output_filepath)
            count += 1
            if max_saved != 0 and count >= max_saved:
                break 
        
        self.last_saved_files[output_filename] = time.time()