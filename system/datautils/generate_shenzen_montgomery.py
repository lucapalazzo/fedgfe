...
from datautils.dataset_shenzhen_montgomery import ShenzhenMontgomeryDataset as Dataset
...
def generate_shenzen_montgomery(args, dir_path, num_clients, num_classes, niid, balance, partition, alpha=0.1, class_per_client = 2):
    ...
    trainset = Dataset(root=dir_path+"rawdata", train=True,  transform=transform)
    testset = Dataset( dataset=trainset, train=False, transform=transform)
    ...
    for _, train_data in enumerate(trainloader, 0):
        # traindata, traintargets, trainmasks, traininfo = train_data['samples'], train_data['masks'], train_data['sample_info']
        traindata, traintargets, trainmasks, traininfo = train_data[1]['samples'], train_data[1]['labels'], train_data[1]['masks'], train_data[1]['info']
    for _, test_data in enumerate(testloader, 0):
        testdata, testtargets, testmasks, testinfo = test_data[1]['samples'], test_data[1]['labels'], test_data[1]['masks'], test_data[1]['info']
    ...
    dataset_info.extend(traininfo)
    dataset_info.extend(testinfo)
    ...
    dataset_union = { 'samples': dataset_image, 'labels': dataset_label, 'masks': dataset_mask, 'info': dataset_info }
...
if __name__ == "__main__":
    ...
    generate_shenzen_montgomery(args, dir_path, num_clients, num_classes, niid, balance, partition )